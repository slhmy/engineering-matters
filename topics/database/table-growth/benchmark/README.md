...
This experiment runs PostgreSQL 17.6 in Docker and creates deterministic data for two table-growth questions:
...
Each case is warmed once and then measured with `EXPLAIN (ANALYZE, BUFFERS, TIMING OFF)`. This exposes execution strategy and work performed, not only elapsed time. The numbers are local observations, not a ranking of database products.
...
- This experiment does not measure insert, secondary-index, or concurrent-write cost.
